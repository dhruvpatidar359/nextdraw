...
const Canvas = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      document
        .getElementById("canvas")
        .removeEventListener("wheel", handler, { passive: false });
    };
  }, []);
  ...
  const handleMouseUp = (event) => {
    ...
    if (action === "drawing") {
      ...
      if (element.type != "pencil") {
        ...
        store.dispatch(
          setElement([tempNewArray, true, element.id.split("#")[0]])
        );
      }
      ...
      if (action === "moving") {
        ...
        if (type === "pencil") {
          ...
          store.dispatch(
            setElement([tempNewArray, true, newElement.id.split("#")[0]])
          );
        }
        ...
        const key =
          currentStateElement[parseInt(selectedElement.id.split("#")[1])];
    ...
    if (tool === "selection") {
      ...
      if (action === "moving") {
        ...
        if (selectedElement.type === "pencil") {
          ...
          const { id, x1, y1, x2, y2, type, text } = resizeElement(
            event,
            elements
          );
          if (type != "pencil") {
  ...
  return (
    <div>
      ...
      <div className="fixed bottom-2 left-2 flex flex-row items-center">
        <TooltipProvider delayDuration={100}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                onClick={() =>
                  setscale((prev) => (prev - 0.1 < 0.1 ? 0.1 : prev - 0.1))
                }
                variant="outline"
                size="icon"
                className="h-8 w-8"
        ...
        <div className="mx-2 bg-[#F6FDC3] rounded-md">
          {" "}
      ...
      {action === "writing" && tool === "text" && selectedElement != null ? (
        <textarea
          ...
          style={{
            position: "fixed",
            top: (selectedElement.y1 + panOffset.y) * scale - scaleOffset.y,
            left: (selectedElement.x1 + panOffset.x) * scale - scaleOffset.x,

            font: `${selectedElement.fontSize * scale}px Virgil`,
            ...
            maxHeight:
              height -
              (selectedElement.y1 + panOffset.y) * scale +
              scaleOffset.y,
            maxWidth:
              width -
              (selectedElement.x1 + panOffset.x) * scale +
              scaleOffset.x,
          }}
        />
      ) : null}
